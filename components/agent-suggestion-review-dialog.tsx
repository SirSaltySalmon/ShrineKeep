...
import { buildApprovedCreatedItems, initialSelectedKeys, persistAgentReviewDraft, switchCreateItemKind } from "@/lib/webmcp/review"
...
interface Props {
  ...
  onPersistReview: (batch: AgentSuggestionBatch) => void
...
export default function AgentSuggestionReviewDialog({ batch, open, applying, error, onOpenChange, onPersistReview, onDiscard, onApplyItemEdits, onApplyCreatedItems, onApplyWishlistPriceEdits }: Props) {
  const [imageSearchOverride, setImageSearchOverride] = useState<boolean | null>(() => batch?.imageSearchOverride ?? null)
  const [selected, setSelected] = useState<Set<string>>(() => initialSelectedKeys(batch))
  ...
  const approvedItemEdits = useMemo(() => itemDrafts.flatMap<ApprovedItemEdit>((entry) => {
    ...
    return change.name === undefined && change.description === undefined && change.current_value === undefined && change.acquisition_price === undefined ? [] : [change]
  }), [itemDrafts, selected])
  ...
  const approvedWishlistPrices = useMemo(() => wishlistDrafts.flatMap<ApprovedWishlistPriceEdit>((entry) => {
    ...
    return change.name === undefined && change.description === undefined && change.current_value === undefined && change.expected_price === undefined ? [] : [change]
  ...
  const persistNow = () => {
    ...
    const entries =
      current.batch.kind === "item_edits"
        ? current.itemDrafts
        : current.batch.kind === "create_items"
          ? current.createDrafts
          : current.wishlistDrafts
    current.onPersistReview(
      persistAgentReviewDraft(current.batch, {
        selectedKeys: Array.from(current.selected),
        imageSearchOverride: current.imageSearchOverride,
        entries,
      })
    )
  }
  ...
  useEffect(() => {
    if (!open) return
    persistNow()
  }, [open, selected, itemDrafts, createDrafts, wishlistDrafts, imageSearchOverride])

  const handleOpenChange = (nextOpen: boolean) => {
    if (!nextOpen) persistNow()
    ...
    <DialogContent className="flex max-h-[90vh] max-w-4xl flex-col overflow-hidden">
      ...
      {batch && <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
        ...
        {batch.kind === "item_edits" && itemDrafts.map((entry, index) => <div key={entry.key} className="space-y-3 rounded-lg border p-4">
          ...
          {(entry.proposedName !== undefined || entry.proposedDescription !== undefined) && <div className="grid gap-3 pl-7 sm:grid-cols-2">

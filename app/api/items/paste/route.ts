...
type PasteItemRequest = PasteItemsBody | PasteRefsBody

function isReferenceMode(body: PasteItemRequest): body is PasteRefsBody {
  ...
  return startRouteSpan(
    ...
    async () => {
      try {
        ...
        let expandedItems: PasteItemsBody["items"] = []
        ...
        console.error("Error creating items paste:", message, error)

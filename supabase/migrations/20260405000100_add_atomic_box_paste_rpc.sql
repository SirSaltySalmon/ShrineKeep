-- Atomic box paste RPC.
-- Inserts a full copied tree (boxes + items + related rows) in one transaction.
-- On any error, Postgres rolls back the entire statement.
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM tmp_items WHERE btrim(name) = '') THEN
  ...
  RETURN jsonb_build_object(
    ...
    'created_items', (SELECT COUNT(*) FROM tmp_items)
  );
END;
$$;
